...
async function fetchSynopsis(url, search) {
    try {
        ...
        $('h2').each((i, elem) => {
            if ($(elem).text().includes(search)) {
                ...
                if (nextElem.is('p')) {
                    ...
                    console.log(`Synopsis trouvé pour ${url}:\n${synopsisText}`);
                    return;
                }
            }
        });
        ...
        console.log('Erreur pendant la récupération du synopsis pour ', url);
        return null;
    }
}
...
async function fetchAllSynopsis(list) {
    ...
    for (const elem of list) {
        const url = `https://anime-sama.fr/catalogue/${String(elem).toLowerCase().replaceAll(' ', '-')}`;
